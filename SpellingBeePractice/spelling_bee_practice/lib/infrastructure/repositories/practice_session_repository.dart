...
class PracticeSessionRepository {
  static Future<List<PracticeSession>> getAllSessions() async {
    ...
    try {
      final List<Map<String, dynamic>> maps =
          ...
          where: 'id > 0' // Excluir sesiones fijas
      );
 ...
 static Future<List<Word>> loadSessionWords(PracticeSession session) async {
    final db = await DBHelper().database;
    try {
      if (session.id == -1) { // Errores
        final List<Map<String, dynamic>> maps = await db.query(
          DBHelper().tableWords,
          where: 'total_incorrect_count > 0', // Solo palabras con errores
          orderBy: 'total_incorrect_count DESC', // Ordenar por errores (descendente)
        );
        return List.generate(maps.length, (i) => Word.fromMap(maps[i]));

      } else if (session.id == -2) { // No Practicadas
            ...
            final List<Map<String, dynamic>> practicedWordsMap = await db.query('practice_history', columns: ['word_id'], distinct: true); //Usamos distinct
            final List<int> practicedWordIds = practicedWordsMap.map((map) => map['word_id'] as int).toList();


            // Filtrar para obtener solo las palabras NO practicadas
            final List<Word> neverPracticedWords = allWords.where((word) => !practicedWordIds.contains(word.id)).toList();
            return neverPracticedWords;


      } else if (session.id == -3) { // Todas
        final List<Map<String, dynamic>> maps = await db.query(
          DBHelper().tableWords,
          orderBy: 'correct_count + incorrect_count ASC', // Menos practicadas primero
        );
        return List.generate(maps.length, (i) => Word.fromMap(maps[i]));

      } else { // Sesiones normales (IDs positivos)
